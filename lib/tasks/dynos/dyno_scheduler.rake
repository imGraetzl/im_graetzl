...
namespace :dyno do
  ...
  task schedule: :environment do
    ...
    def scale_dyno(heroku, type, target_size, target_quantity, target_time)
      # Beide Werte für den Vergleich in Kleinbuchstaben umwandeln
      ...
      Rails.logger.info("Current configuration for #{type} dyno: Size=#{current_config['size']}, Quantity=#{current_quantity}")
      Rails.logger.info("Target configuration for #{type} dyno: Size=#{target_size.capitalize}, Quantity=#{target_quantity}")
      ...
      if current_size == target_size && current_quantity == target_quantity
        ...
        begin
          ...
          Rails.logger.error("Parameters used: Size=#{target_size.capitalize}, Quantity=#{target_quantity}")
        end
      end
    end
    ...
    def schedule_dyno_configurations(heroku)
      ...
      if hour >= 23 || hour < 7
        scale_dyno(heroku, 'web', 'Standard-2X', 1, night_target_time)
      else
        scale_dyno(heroku, 'web', 'Standard-2X', 2, day_target_time)
      end
    end
