namespace :scheduled do
  ...
  task weekly_summary_mail: :environment do
    ...
    User.confirmed.find_each do |user|
      Region.all.each do |region|
        NotificationMailer.summary_graetzl(user, region, 'weekly').deliver_now if Date.today.thursday?
        NotificationMailer.summary_personal(user, region, 'weekly').deliver_now if Date.today.thursday?
      end
    end
  end
