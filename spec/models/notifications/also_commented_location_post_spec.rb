...
RSpec.describe Notifications::AlsoCommentedLocationPost, type: :model do

  describe '.triggered_by?(activity)' do
    let(:activity) { build :activity, key: 'location_post.comment' }
    let(:other_activity) { build :activity, key: 'location_post.create' }

    it 'returns true if activity.key matches TRIGGER_KEY' do
      expect(Notifications::AlsoCommentedLocationPost.triggered_by?(activity)).to eq true
    end

    it 'returns false if activity.key does not match TRIGGER_KEY' do
      expect(Notifications::AlsoCommentedLocationPost.triggered_by?(other_activity)).to eq false
    end
  end
  ...
  describe '.receivers(activity)' do
    ...
    subject(:receivers) { Notifications::AlsoCommentedLocationPost.receivers(activity) }
  ...
  describe '.condition(activity)' do
    ...
    subject(:condition) { Notifications::AlsoCommentedLocationPost.condition activity }
