APP.controllers_loggedin.energy_demands = (function() {

  function init() {
    if ($("section.energy-demand-form").exists()) initEnergyForm();
  }
  ...
  function initEnergyForm() {
    ...
    // show eeg sub_types
    $('input[name="energy_demand[energy_type]"]').on("change", function() {

      show_beg(true);

      if ($(this).is(':checked') && ["eeg"].includes($(this).val())) {
        $(".eeg-types").show();
        $("#energy_demand_energy_type_eeg").prop('checked', false).addClass("checked");
        $("#energy_demand_energy_type_eeg_local").prop('checked', true);
      } else if ($(this).is(':checked') && ["eeg_local", "eeg_regional"].includes($(this).val())) {
        $(".eeg-types").show();
        $("#energy_demand_energy_type_eeg").prop('checked', false).addClass("checked");
      } else if ($(this).is(':checked') && ["beg"].includes($(this).val())) {
        $(".eeg-types").hide();
        $("#energy_demand_energy_type_eeg").prop('checked', false).removeClass("checked");
        show_beg(false);
    ...
    $('input[name="energy_demand[energy_category_ids][]"]').on("change", function() {
      if ($(this).is(':checked')) {
